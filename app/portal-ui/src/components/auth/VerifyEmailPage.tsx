...
export const VerifyEmailPage = () => {
  ...
  const showNotification = useNotificationStore((state) => state.show); // 👈 NEW
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        ...
        showNotification("Account verified! You are ready to explore.", "success");
      } catch (error) {
        setStatus('error');
      }
    };
      ...
      <div className="max-w-md w-full bg-white border-4 border-slate-900 p-10 rounded-[2.5rem] shadow-[12px_12px_0px_0px_#0f172a] text-center space-y-6">
        ...
        {status === 'loading' && (
          <div className="space-y-4">
            ...
            <h1 className="text-2xl font-black text-slate-900 uppercase italic">Calibrating Compass...</h1>
            <p className="font-bold text-slate-500 text-sm uppercase tracking-widest">Checking your credentials</p>
          </div>
        )}
        ...
        {status === 'success' && (
          <div className="space-y-6 animate-in zoom-in duration-500">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-black text-slate-900 uppercase italic">Ready to Explore!</h1>
              <p className="font-bold text-slate-500 text-sm">Your enrollment gate is now open. Let's get started!</p>
            </div>
            <Button 
              onClick={() => navigate('/dashboard/register')}
              className="w-full py-6 bg-emerald-500 text-white border-4 border-slate-900 rounded-2xl font-black shadow-[6px_6px_0px_0px_#0f172a] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all"
            >
              GO TO REGISTRATION →
            </Button>
          </div>
        ...
        {status === 'error' && (
          <div className="space-y-6 animate-in shake duration-500">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-black text-slate-900 uppercase italic">Signal Lost!</h1>
              <p className="font-bold text-slate-500 text-sm">This link is invalid or has already been used.</p>
            </div>
            <Button 
