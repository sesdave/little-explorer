...
import { useSessionClasses } from '@/hooks/use-sessions';
...
export const SessionBuilderPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="p-10 font-black animate-pulse">
        Loading Inventory...
      </div>
    );
  }
      ...
      <div className="flex justify-between items-end">
        <div>
          <button
            ...
            className="text-slate-400 hover:text-slate-900 flex items-center gap-2 text-xs font-black uppercase mb-4"
          >
          ...
          <h1 className="text-4xl font-black uppercase italic tracking-tighter text-slate-900">
            {sessionName || 'Session'}{' '}
            <span className="text-sky-400 not-italic">Inventory</span>
          </h1>
        </div>

        {/* Add button always allowed (better UX) */}
        {classes.length > 0 && (
        ...
        {classes.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center p-20 border-4 border-dashed border-slate-200 rounded-[3rem] bg-slate-50/50">
            ...
            <p className="font-black uppercase text-slate-400 mb-6 tracking-widest text-center">
              ...
              <span className="text-[10px] font-bold">
                Start by adding your first class template.
              </span>
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {classes.map((cls: any) => (
              <div
                key={cls.id}
                className="p-6 bg-white border-4 border-slate-900 rounded-3xl shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]"
              >
